    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <?php 
          ...
          $req = "select * from agent order by NomAgent";
$resu = $pdo->query($req);
...
if(strpos($fullUrl, "suppression=valid") == true) {
  echo "<h6  style='text-align:center; color: red;' class='errorempty'>L'agent Est bien supprimer</h6>";
}
if(strpos($fullUrl, "suppression=echec") == true) {
    echo "<h6 style='text-align:center;' class='errorempty'> Impossible de supprimer l'agent</h6>";
}
if(strpos($fullUrl, "Erreur=correcte") == true) {
    echo "<h6 style='text-align:center;' class='success'> Verifier Votre Boite Email</h6>";
}
if(strpos($fullUrl, "enregistre=success") == true) {
  echo "<h6 style='text-align:center;' class='success'>L'agent est bien modifier</h6>";
}
...
if(strpos($fullUrl, "enregi=succes") == true) {
  echo "<h6 style='text-align:center;' class='success'>L'agent est bien enregistre</h6>";
}
           ?>
</div>
    <div style="padding-top: 1%;" class="custyle">
      <div style="text-align: bottom; margin-bottom: -4%;">
      <a class="btn btn-info btn-xs" href="ajoutagent.php"><span class='glyphicon glyphicon-edit'></span>Ajouter Agent</a></div>
    <table class="table table-striped custab text-center">
    
        <tr>
            <th class='tr1' >Nom agent</th>
            <th class='tr1' >Prenom agent</th>
            <th class='tr1'>Téléphone agent </th>
            <th class='tr1' >Email agent </th>
       ...
       foreach ($affi as $dat)
      if ($dat->IdAgent != $_SESSION['IdAgent']) {
        if ($dat->EmailAgent != 'admin') {
      echo "<tr>
        <td class='tr1'>"
           . $dat->NomAgent .
        "</td>
        ...
        "</td>
         ...
         <td class='text-center'> <a class='btn btn-warning btn-xs' href='modifierlesagent.php?id=$dat->IdAgent'><span class='glyphicon glyphicon-edit'></span>Modifier</a> <a class='btn btn-danger' href='supprimerlesagents.php?id=$dat->IdAgent'><span class='glyphicon glyphicon-edit'></span>Supprimer</a> </td> </tr>";
         }}?>
          <tbody id="result">
