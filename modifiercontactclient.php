...
$requete4 = "select * from contactclient NATURAL JOIN client WHERE IdContactClient like " . $_SESSION['id'] . "";
 ...
 <div id="wrapper">
    ...
    <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
      ...
      <li class="nav-item active">
        <a class="nav-link" href="acceuil?email=<?php echo($_SESSION['EmailAgent']);?>&pass=<?php echo($_SESSION['PasswordAgent']);?>">
      ...
      <div class="modal-footer">
        ...
        <div class="container" style="min-height: 100px; margin-top: 4%;">
			<div class="main">
				...
				<h3 class="deb3">Modfier Le Contact Client <?php echo($key->NomContactClient);?></h3>
					<form class="" method="get" action="modifiercontactclientph.php">
          
            <div class="form-group">

              <label for="Votre Nom">Nom contact client</label>
                <div class="input-group">
                  
                  <input type="text" class="form-control" value="<?php echo($key->NomContactClient);?>" name="nomontactclient" placeholder="Nom Contact Client"/>
                </div>
            </div>
            <div class="form-group ">
              <label for="entreprise">Téléphone contact client</label>
                <div class="input-group">
                  <input type="phone" class="form-control" value="<?php echo($key->TelContactClient);?>" name="tel" placeholder="Telephone"/>
              </div>
            </div>

            <div class="form-group">
              <label for="La Date de Début">Email contact client</label>
                <div class="input-group">
                  <input type="email" class="form-control" value="<?php echo($key->EmailContactClient);?>" name="email" placeholder="Email"/>
                </div>
            </div>
            <div class="form-group">
              <label for="Nom de License">Client</label>
                <div class="input-group">
                  
                 <select class="form-control" name="client" id="metiers" class="tg" id="txtville" >
                    <option value="<?php echo($key->IdClient);?>"><?php echo($key->NomClient);?></option>
     <?php  
     foreach ($metier as $data)
      if (($key->IdClient != $data->IdClient)) {
        
      
      echo "<option value=" . $data->IdClient . ">" . $data->NomClient .  "</option>";}
      ?>
                </select>
                </div>
            </div>
...
elseif(strpos($fullUrl, "enregistre=success") == true) {
  ...
  echo "<h4 class='success'>Votre Contact Client " . ucwords($_GET['nomontactclient']) . "  Est Bien Modifier</h4>";
}
