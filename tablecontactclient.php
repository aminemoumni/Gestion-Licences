    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <?php 
          ...
          $req = "select * from client NATURAL JOIN contactclient order by IdContactClient DESC";
$resu = $pdo->query($req);
$affi = $resu->fetchAll(PDO::FETCH_OBJ);
    ...
    <a class="btn btn-info btn-xs" href="ajoutproduit.php"><span class='glyphicon glyphicon-edit'></span>Ajouter produit</a>
    <a style="float: right;" class="btn btn-info btn-xs" href="ajoutlicence.php"><span class='glyphicon glyphicon-edit'></span>Ajouter licence</a></div>
  <div class="text-center" style="margin-bottom: -3%;">
  <?php 
$fullUrl = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";          

if(strpos($fullUrl, "suppression=valid") == true) {
  echo "<h6  style='text-align:center; color: red;' class='errorempty'>Votre Contact Client Est Bien Supprimer</h6>";
}
if(strpos($fullUrl, "suppression=echec") == true) {
  echo "<h6  style='text-align:center; color: red;' class='errorempty'>Impssible de Supprimer le contact client deja enregistre dans une licence</h6>";
}        
...
elseif(strpos($fullUrl, "enregistre=char") == true) {
  echo "<h6 class='errorempty'>veuillez remplir Nom Contact Client Correctement</h6>";
}
elseif(strpos($fullUrl, "enregistre=mail") == true) {
  echo "<h6 class='errorempty'> Email Invalid</h4>";
}
elseif(strpos($fullUrl, "enregistre=tel") == true) {
  echo "<h6 class='errorempty'> Numero De Telephone Invalid</h4>";
}
elseif(strpos($fullUrl, "enregistre=typ") == true) {
  echo "<h6 class='errorempty'> Choissiez un client </h4>";
}
elseif(strpos($fullUrl, "enregistre=same") == true) {

  echo "<h4 class='errorempty'>Email Deja Enregistre</h4>";
}
    ...
    <table class="table table-striped custab text-center">
        ...
        <tr>
            <th class='tr1' >Nom Contact Client</th>
            <th class='tr1' >Téléphone</th>
      ...
      echo "<tr>
        <td class='tr1'>"
           . $dat->NomContactClient .
        "</td>
        <td class='tr1'>"
           . $dat->TelContactClient .
        "</td>
        
        
        <td class='tr1'>"
          . $dat->EmailContactClient .
        "</td>
        <td class='tr1'>"
           . $dat->NomClient .
        "</td>
        
         <td class='text-center'> <a class='btn btn-warning btn-xs' href='modifiercontactclient.php?id=$dat->IdContactClient'><span class='glyphicon glyphicon-edit'></span>Modifier</a> <a class='btn btn-danger' href='supcontactclient.php?id=$dat->IdContactClient'><span class='glyphicon glyphicon-edit'></span>Supprimer</a> </td> </tr>"
        ?>
          <tbody id="result">
