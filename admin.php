...
if(strpos($fullUrl, "sup=clean") == true) {
 ...
 $limit = 10;
               ...
               $start = ($page - 1) * $limit;
               $requettt = "SELECT count(IdTraceLicence) as idli FROM tracelicence";
    ...
    <div style="padding-top: 1%;" class="custyle">
      ...
      <a class="btn btn-info btn-xs" href="ajoutlicence.php"><span class='glyphicon glyphicon-edit'></span>Ajouter Licence</a></div>
<table class="table table-striped custab text-center">
  <tr>
            ...
            <th class="text-center">Action</th>
               ...
               $page = $_GET['page']; } 
                $requete1 = "select IdTraceLicence, IdLicence, NomClient, DesignationProduit, LibelleTypeLicence, Garantie, DateDebut, DateFin, NomAgent, PrenomAgent, NomEnregistrement, NomContactClient, Numerodeserie, ImageLicence, ReferenceMarche
from tracelicence NATURAL JOIN agent NATURAL JOIN client NATURAL JOIN contactclient NATURAL JOIN produit NATURAL JOIN typelicence Where Validation = 'Active' order by IdTraceLicence DESC LIMIT $start,$limit";
...
$affi = $resu->fetchAll(PDO::FETCH_OBJ);
      ...
      echo "<tr>
        ...
        "</td>
         ...
         <td class='text-center'><a class='btn btn-info btn-xs' href='plusinfotrace.php?id=$dat->IdTraceLicence&page=$page'><span class='glyphicon glyphicon-edit'></span>Plus Info</a> "
        ?>
        </table>
  ...
  <ul class="pagination">
    <li class="page-item"><a class="page-link" href="admin.php?&page=<?php echo($p); ?>">Précedent</a></li>
   <?php for($i = 1; $i <= $pages; $i++) : ?>
            <li class="page-item"><a class="page-link" href="admin.php?&page=<?php echo($i); ?>"><?php echo($i); ?></a></li>
            <?php endfor; ?>
   
    <li class="page-item"><a class="page-link" href="admin.php?page=<?php echo($s); ?>">Suivant</a></li>
  </ul>
</nav>
</div>     
                    </div>
